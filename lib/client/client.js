(function () {
    ...
    var checkEndGame = function () {
        if (game.inCheck(game.turn)) {
            log(logColor(game.turn) + " is in check!");
        }

        if (game.inCheckMate(game.turn)) {
            log("Checkmate! " + logColor(!game.turn) + " wins.");
            hasEnded = true;
    ...
    var performMove = function (piece, from, to) {
        ...
        } else if (p instanceof chess.chess_Pawn && t.r === (white? 7: 0)) {
